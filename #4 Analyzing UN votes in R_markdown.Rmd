...
by_country %>% 
  ...
  labs(x = "% of yes votes in UN",
       title = "What countries voted 'yes' the most and least")
```
...
unvotes %>% 
  ...
  summarize(coorelation = cor(Canada, India))
...
country_correlations %>% 
  ...
  arrange(desc(avg_intracontinent_correlation))
```
...
unvotes %>% 
  inner_join(rc_words, by = "rcid") %>% 
  filter(country %in% c("Germany", "Israel")) %>% 
